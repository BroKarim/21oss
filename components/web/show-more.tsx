...
export const ShowMore = <T,>({ items, limit = 4, buffer = 1, showMoreType = "link", renderItem, ...props }: ShowMoreProps<T>) => {
  ...
  return (
    <Stack {...props}>
      ...
      {!shouldShowAll && hiddenCount > 0 && (
        <>
          {showMoreType === "link" && (
            <button type="button" className="font-medium text-sm text-muted-foreground hover:text-foreground" onClick={() => setShowAll(true)}>
              +{hiddenCount} more
            </button>
          )}
