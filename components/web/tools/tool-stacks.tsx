...
import { BrandLink } from "@/components/web/brand-link";
...
export const ToolStacks = ({ stacks, limit = 12, ...props }: ToolStacksProps) => {
  ...
  return <ShowMore items={stacks} limit={limit} renderItem={(stack) => <BrandLink key={stack.slug} href={`/stacks/${stack.slug}`} name={stack.name} faviconUrl={stack.faviconUrl} />} {...props} />;
};
