...
const ToolRefinementComponent = ({ filter, items, isPending, disabled, className, ...props }: ToolRefinementProps) => {
  const { filters, updateFilters } = useFilters();
  const selectedValues = filters[filter];

  return (
    <Command filter={(value, search) => (value.toLowerCase().includes(search.toLowerCase()) ? 1 : 0)} className={cx("size-full border rounded-md", className)} {...props}>
      <CommandInput placeholder={`Search ${filter}`} className="w-full text-xs! min-w-[0]! px-3 py-2 font-normal border-b outline-none disabled:opacity-50" disabled={disabled} />
        ...
        <CommandGroup>
          ...
          {items.map((item: FilterOption) => (
            <CommandItem
              ...
              onSelect={() =>
                updateFilters({
                  [filter]: selectedValues.includes(item.slug) ? selectedValues.filter((v) => v !== item.slug) : [...selectedValues, item.slug],
                })
              }
              className={cx(
              ...
              disabled={disabled}
            >
              <input type="checkbox" checked={selectedValues.includes(item.slug)} readOnly className="pointer-events-none" />
