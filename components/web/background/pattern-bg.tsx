...
const BGPattern = ({ variant = "grid", mask = "none", size = 24, fill = "#ffff", className, style, ...props }: BGPatternProps) => {
  const bgSize = `${size}px ${size}px`;
  const backgroundImage = geBgImage(variant, fill, size);

  return (
    <div
      className={cn("absolute opacity-50 inset-0 z-[-10] size-full", maskClasses[mask], className)}
      style={{
        backgroundImage,
