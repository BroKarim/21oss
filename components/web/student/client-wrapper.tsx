      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <AnimatePresence mode="wait">
          {filteredData.length > 0 ? (
            <m.div key="table" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <StuffTable data={filteredData} />
            </m.div>
          ) : (
            <m.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20 text-muted-foreground bg-muted/10 rounded-xl border border-dashed border-border/50">
