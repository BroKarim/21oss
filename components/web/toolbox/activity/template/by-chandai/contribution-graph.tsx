...
import { createContext, type CSSProperties, Fragment, type HTMLAttributes, type ReactNode, useContext, useMemo } from "react";
...
const groupByWeeks = (activities: Activity[], weekStart: WeekDay = 0): Week[] => {
  ...
  const paddedActivities = [...(new Array(differenceInCalendarDays(firstDate, firstCalendarDate)).fill(undefined) as Activity[]), ...normalizedActivities];
...
export const ContributionGraphCalendar = ({ hideMonthLabels = false, className, children, ...props }: ContributionGraphCalendarProps) => {
...
export const ContributionGraphFooter = ({ className, ...props }: ContributionGraphFooterProps) => <div className={cn("flex flex-wrap gap-1 whitespace-nowrap sm:gap-x-4", className)} {...props} />;
