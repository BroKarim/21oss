...
import { cx, cva, VariantProps } from "@/lib/utils";
...
export const PaginationLink = ({ children, className, prefix, suffix, isActive, isDisabled, ...props }: PaginationLinkProps) => {
  if (isDisabled) {
    return (
  ...
  return (
    <Link className={cx(isActive && "bg-accent rounded-xs", navLinkVariants({ isActive, className }))} {...props}>
      <Slot.Root className={affixVariants()}>{prefix}</Slot.Root>
      <span>{children}</span>
      <Slot.Root className={affixVariants()}>{suffix}</Slot.Root>
    </Link>
