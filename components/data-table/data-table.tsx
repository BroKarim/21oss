...
export function DataTable<TData>({ table, floatingBar = null, emptyState = "No results.", children, className, ...props }: DataTableProps<TData>) {
  ...
  return (
    <>
      ...
      <Table className={cx("rounded-md border", className)} style={{ "--table-columns": gridColumns.join(" ") } as CSSProperties} {...props}>
            ...
            table.getRowModel().rows.map((row) => (
              <TableRow key={row.id} data-state={row.getIsSelected() && "selected"}>
                {row.getVisibleCells().map((cell) => (
                  <TableCell key={cell.id} style={getColumnPinningStyle({ column: cell.column, withBorder: true })}>
