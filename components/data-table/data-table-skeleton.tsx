...
export function DataTableSkeleton(props: DataTableSkeletonProps) {
  const { title, columnCount = 5, rowCount = 10, searchableColumnCount = 1, filterableColumnCount = 2, showViewOptions = true, cellWidths = ["auto"], withPagination = true, shrinkZero = true, className, ...skeletonProps } = props;
    ...
    <div className={cx("w-full space-y-4 overflow-auto", className)} {...skeletonProps}>
      ...
      <div className="flex w-full items-center justify-between space-x-2 overflow-auto">
        <div className="flex flex-1 items-center space-x-2">
          {searchableColumnCount > 0 && Array.from({ length: searchableColumnCount }).map((_, i) => <Skeleton key={i} className="h-8 w-40 lg:w-60" />)}

          {filterableColumnCount > 0 && Array.from({ length: filterableColumnCount }).map((_, i) => <Skeleton key={i} className="h-8 w-18 border-dashed" />)}
        </div>

        {showViewOptions && <Skeleton className="ml-auto hidden h-8 w-18 lg:flex" />}
      </div>

      <Table className="rounded-md border" style={{ "--table-columns": `repeat(${columnCount}, minmax(0, 1fr))` } as CSSProperties}>
        <TableHeader>
