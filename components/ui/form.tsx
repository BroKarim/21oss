...
const FormControl = (props: ComponentProps<typeof Slot.Root>) => {
  ...
  return <Slot.Root id={formItemId} aria-describedby={!error ? `${formDescriptionId}` : `${formDescriptionId} ${formMessageId}`} aria-invalid={!!error} {...props} />;
};

const FormDescription = ({ className, ...props }: ComponentProps<"p">) => {
  const { formDescriptionId } = useFormField();

  return <p id={formDescriptionId} className={cx("text-[0.8rem] text-foreground/50", className)} {...props} />;
};
