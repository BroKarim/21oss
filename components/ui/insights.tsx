...
export const Insights = ({ className, insights, ...props }: InsightsProps) => {
  return (
    <ul className={cn("w-full text-xs", className)} {...props}>
        ...
        return (
          <Fragment key={label}>
            <li className="flex items-center gap-3 py-1">
              ...
              {link ? (
                <Link href={link} className="shrink-0 font-medium decoration-border underline hover:decoration-foreground" title={title}>
                  {value}
                </Link>
              ) : (
