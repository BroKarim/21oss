...
interface RainbowButtonProps extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, "prefix" | "suffix">, VariantProps<typeof rainbowButtonVariants> {
  asChild?: boolean;
  prefix?: React.ReactNode;
  suffix?: React.ReactNode;
}

const RainbowButton = React.forwardRef<HTMLButtonElement, RainbowButtonProps>(({ className, variant, size, asChild = false, prefix, suffix, children, ...props }, ref) => {
  const Comp = asChild ? Slot : "button";

  return (
    <Comp data-slot="button" className={cn("inline-flex items-center gap-2", rainbowButtonVariants({ variant, size, className }))} ref={ref} {...props}>
