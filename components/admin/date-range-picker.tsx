...
export function DateRangePicker({ defaultDateRange, placeholder = "Pick a date", triggerClassName, shallow = false, className, ...props }: DateRangePickerProps) {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button className={cx("shrink-0 justify-start truncate text-left", !date && "text-muted-foreground", triggerClassName)}>
          {date?.from ? date.to ? `${formatDateRange(date.from, date.to)}` : `${formatDate(date.from)}` : <span>{placeholder}</span>}
        </Button>
      </PopoverTrigger>
