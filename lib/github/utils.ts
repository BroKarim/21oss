...
//Ini digunakan untuk mengekstrak bagian:
// owner: misal shadcn-ui
// name: misal ui => Dari link seperti: https://github.com/shadcn-ui/ui

export const githubRegex = /^(?:https?:\/\/)?github\.com\/(?<owner>[^/]+)\/(?<name>[a-zA-Z0-9._-]+?)(?:[/?#]|$)/;
 ...
 */
export const calculateHealthScore = ({ stars, forks, contributors, watchers, createdAt, pushedAt }: GetToolScoreProps) => {
 ...
 */
export const prepareRepositoryData = async (repository: RepositoryQueryResult): Promise<RepositoryData> => {
  const metrics = {
  ...
  const license = !repository.licenseInfo || repository.licenseInfo.spdxId === "NOASSERTION" ? null : repository.licenseInfo.spdxId;
