...
// ambil metadata dari GitHub repository menggunakan GitHub GraphQL AP
export const githubClient = {
  async queryRepository(repository: string): Promise<RepositoryData | null> {
    const repo = getRepository(repository);

    try {
      const { repository } = await client<{ repository: RepositoryQueryResult }>(repositoryQuery, repo);
