...
import { AppSection, MainTabType, currentSectionAtom, selectedMainTabAtom, getMainPageUrlWithTab } from "@/lib/atoms";
import { useMediaQuery } from "@/hooks/use-media-query";
import type { SortOption } from "@/types/globals";
...
export function useNavigation(options: UseNavigationOptions = {}): NavigationResult {
  const { syncWithUrl = true, onTabChange, useResponsiveDefaults = true } = options;
  ...
  }, [pathname, urlTab, isDesktop, setCurrentSection, setSelectedMainTab, useResponsiveDefaults, syncWithUrl, router, searchParams]);
  ...
  const navigateToTab = (tab: MainTabType | "home") => {
    ...
    if (syncWithUrl) {
      const url = getMainPageUrlWithTab(tab, tab === "components" ? sortBy : undefined);
      router.push(url);
    }
  };
